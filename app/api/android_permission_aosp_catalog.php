...
try {
    $filters = [
        ...
        'bucket' => get_enum('bucket', perm_bucket_keys(), null),
    ];

    $payload = db_android_permission_catalog_aosp($filters);
    api_ok($payload['data'] ?? [], $payload['meta'] ?? []);
} catch (Throwable $e) {
    api_error('Failed to load AOSP permissions catalog.', 500, 'ERR_PERMISSION_AOSP', [], $e);
}
