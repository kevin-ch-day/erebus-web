...
import type {
  ...
  TriageStatusOption,
  TriageTableOptions,
} from '../../types/perm-triage-globals';
...
if (window.App && window.PermissionIntel) {
  ...
  const formatUtcFixed = (value: unknown) => App.formatUtc(value, { timeZone: 'UTC' });
  ...
  PermTriage.applyFilters = (rows: TriageRow[], filters: TriageFilters) => {
  ...
  PermTriage.emptyMessage = (unknownRows: TriageRow[], healthData: Record<string, unknown>) => {
  ...
  PermTriage.populateStatusFilter = (statusEl, triageStatuses: TriageStatusOption[]) => {
    if (!statusEl) return;
  ...
  PermTriage.renderUnknownTable = (rows: TriageRow[], options: TriageTableOptions) => {
  ...
  PermTriage.renderCurrentEvidenceTable = (rows: TriageRow[], options: TriageTableOptions) => {
    const {
      bodyEl,
      onReview,
      onEvidence,
  ...
  PermTriage.renderLedgerDiagnosticsTable = (rows: TriageRow[], options: TriageTableOptions) => {
    const {
      bodyEl,
      onInspect,
      emptyMessage,
