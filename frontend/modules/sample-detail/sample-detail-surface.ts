...
if (window.App && SampleDetail) {
  ...
  SampleDetail.createSurface = (elements) => {
    ...
    const hasDisplayValue = SampleDetail.hasDisplayValue || ((value: unknown) => {
      return value !== null && value !== undefined && String(value).trim() !== '' && String(value).trim() !== '--';
    });

    function rowLabel(row: unknown): string {
      return Array.isArray(row) ? String(row[0]) : String((row as { label: unknown }).label);
    }

    function rowValue(row: unknown): unknown {
      return Array.isArray(row) ? row[1] : (row as { value: unknown }).value;
    }

    function addCard(target: HTMLElement | null, title: string, rows: unknown[], options: Record<string, unknown> = {}): void {
      if (!target) return;
      ...
      if (typeof options.className === 'string' && options.className) {
        String(options.className)
          .split(/\s+/)
          .filter(Boolean)
          .forEach((name) => card.classList.add(name));
      }
      const rowClass = options.rowClass ? ' detail-row-vertical' : '';
      const list = filteredRows.map((row) => {
        const label = rowLabel(row);
        const value = rowValue(row);
        const objectRow = !Array.isArray(row) ? row as { rendered?: string; valueClass?: string } : null;
        const rendered = objectRow && objectRow.rendered !== undefined ? objectRow.rendered : esc(fmt(value));
    ...
    function backoffNote(statusRaw: unknown, nextEligibleRaw: unknown): string {
    ...
    function boolLabel(value: unknown, yes = 'Yes', no = 'No'): string {
      return value ? yes : no;
    }

    function normalizeFamilyValue(value: unknown): string {
      return String(value || '').trim().toLowerCase();
    }

    function familySignalStatus(sample: Record<string, unknown>): string {
    ...
    function pillToneForAlignment(value: unknown): string {
    ...
    function pillToneForVtStatus(value: unknown): string {
    ...
    function badgeMarkup(value: unknown, tone: string): string {
      return `<span class="sample-status-pill ${esc(tone)}">${esc(fmt(value))}</span>`;
    }

    function boolBadge(value: unknown, options: Record<string, unknown> = {}): string {
    ...
    function syncSectionVisibility(sectionEl: HTMLElement | null, contentEl: HTMLElement | null, extraVisible = false): void {
      if (!sectionEl) return;
    ...
    function updateHeader(sample: Record<string, unknown>): void {
    ...
    function renderPlatformContext(platformContext: Record<string, unknown> | null | undefined): void {
    ...
    function renderLastError(row: Record<string, unknown>): void {
    ...
    function renderSample(data: { sample: Record<string, unknown>; last_run?: Record<string, unknown> | null; platform_context?: Record<string, unknown> | null }) {
