import type { AppSurface, AppJsonFailure, AppJsonSuccess } from '../../types/app-globals';
import type {
...
type PermissionControllerElements = Record<string, HTMLElement | HTMLSelectElement | HTMLButtonElement | null>;
type PermissionControllerEndpoints = { permSummaryEndpoint?: string; permDetailEndpoint?: string };
...
if (App && SampleDetail) {
  ...
  SampleDetail.createPermissionsController = (elements: PermissionControllerElements, endpoints: PermissionControllerEndpoints) => {
    const {
      permSummaryList,
      permSummaryEmpty,
      permDetailBody,
    ...
    let permissionRows: SamplePermissionRow[] = [];
    let summaryRows: SamplePermissionSummaryRow[] = [];
    let exportSampleId = '';
    ...
    function bucketDisplay(row: SamplePermissionRow): { label: string; title: string } {
    ...
    function isUnknown(row: SamplePermissionRow): boolean {
      const bucketRaw = fmt(row.bucket, '').trim();
      if (bucketRaw === '') return true;
      return bucketRaw.toUpperCase() === 'UNKNOWN';
    }

    function knownBadge(row: SamplePermissionRow): string {
      if (isUnknown(row)) {
        const classification = String(row.classification || '').toUpperCase();
    ...
    function setPipelineStage(stage: string, state: string): void {
    ...
    function updatePipelineStages(): void {
    ...
    const renderers = SampleDetail.createPermissionsRenderers({
      ...
      permDetailBody,
      permTilesEl,
      permFilterBucket: permFilterBucket as HTMLSelectElement | null,
      permUnknownList,
    }, {
      bucketLabel,
      bucketDisplay,
    ...
    function applyFilters(rows: SamplePermissionRow[]): SamplePermissionRow[] {
    ...
    function updatePermissionTable(): void {
    ...
    function renderFailure(prefix: string, response: AppJsonFailure): void {
      if (!permErrorEl) return;
      permErrorEl.innerHTML = `<pre>${prefix}\n\nHTTP ${response.status}\nerror: ${esc(response.error)}</pre>`;
    }

    function responseRows<T = SamplePermissionRow>(response: AppJsonSuccess): T[] {
      const body = response.body as { data?: unknown };
      return Array.isArray(body?.data) ? body.data as T[] : [];
    }

    async function loadPermissions(sampleIdValue: unknown): Promise<void> {
      ...
      try {
        ...
        if (!summaryRes.ok) {
          ...
          summaryRows = responseRows<SamplePermissionSummaryRow>(summaryRes);
        ...
        if (!detailRes.ok) {
          ...
          permissionRows = responseRows<SamplePermissionRow>(detailRes);
          renderers.renderPermTiles(permissionRows);
          renderers.populateBucketFilter(permissionRows);
          renderers.renderUnknownList(permissionRows);
    ...
    return {
      ...
      showNonAndroid,
      resetEmpty,
      setExportSampleId: (value: unknown) => { exportSampleId = String(value || ''); },
    };
  };
}
