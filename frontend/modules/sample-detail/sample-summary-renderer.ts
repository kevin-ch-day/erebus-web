...
if (App && SampleDetail) {
  ...
  const fmt = App.fmt;
  const fmtUtc = SampleDetail.fmtUtc || ((value: unknown) => value ? App.formatUtc(value) : '--');
  const hasDisplayValue = SampleDetail.hasDisplayValue || ((value: unknown) => {
    return value !== null && value !== undefined && String(value).trim() !== '' && String(value).trim() !== '--';
  });

  function buildRows(items: SummaryRow[]): string {
    return items
      .filter((row) => row && row.keep !== false)
      .map((row) => {
        const value = row.rendered !== undefined ? row.rendered : esc(fmt(row.value));
        return `<div class="detail-row"><div class="detail-label">${esc(row.label)}</div><div class="detail-value${row.mono ? ' mono' : ''}">${value}</div></div>`;
      }).join('');
  }

  function buildHashLines(sample: Record<string, unknown>): string {
    const shaValue = fmt(sample.sha256);
    const hashRows: SummaryRow[] = [
    ...
    return hashRows
      .filter((row) => row.copy || hasDisplayValue(row.value))
      .map((row) => {
  ...
  function buildPanel(title: string, rows: string, className = ''): string {
