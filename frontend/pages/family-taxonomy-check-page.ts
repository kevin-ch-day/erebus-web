...
type RemediationSummary = {
  priority_lanes?: unknown;
  math?: unknown;
  mismatch_pair_classes?: unknown;
  row_pattern_summary?: unknown;
  top_mismatch_pairs?: unknown;
};
...
type PlatformInventory = {
  ...
  platform_alignment_counts?: unknown;
  platform_decision_counts?: unknown;
  platform_held_mismatch_counts?: unknown;
  platform_repair_now_counts?: unknown;
};

type AskWhyInventory = {
  decision_mode?: unknown;
  total_rows?: unknown;
  issue_kind_counts?: unknown;
  platform_counts?: unknown;
  issue_platform_counts?: unknown;
};

type CheckMeta = JsonRecord & {
...
type CheckData = JsonRecord & {
  summary?: unknown;
  remediation_summary?: unknown;
  decision_inventory?: unknown;
  platform_inventory?: unknown;
  ask_why_inventory?: unknown;
  queue_presets?: unknown;
  schema_missing?: unknown;
};

const root = document.getElementById('family-taxonomy-page') as HTMLElement | null;
...
if (root && window.App) {
  ...
  if (endpoint) {
    const searchInput = document.getElementById('family-taxonomy-search') as HTMLInputElement | null;
    const alignmentSelect = document.getElementById('family-taxonomy-alignment') as HTMLSelectElement | null;
    const platformSelect = document.getElementById('family-taxonomy-platform') as HTMLSelectElement | null;
    const patternSelect = document.getElementById('family-taxonomy-pattern') as HTMLSelectElement | null;
    const limitSelect = document.getElementById('family-taxonomy-limit') as HTMLSelectElement | null;
    const refreshBtn = document.getElementById('family-taxonomy-refresh') as HTMLButtonElement | null;
    ...
    function badgeClassForSeverity(value: unknown): string {
      const key = String(value || '').toLowerCase();
    ...
    function queueParams(extra: Record<string, unknown> = {}): Record<string, unknown> {
    ...
    function renderSummary(rows: SummaryRow[]): void {
      if (!summaryEl) return;
      if (!rows.length) {
    ...
    function renderOperationalSplit(inventory: DecisionInventory): void {
      if (!operationalEl) return;
      const modeCounts = mapEntries(inventory.decision_mode_counts)
        .sort((a, b) => Number(b[1] || 0) - Number(a[1] || 0));
      const priorityCounts = mapEntries(inventory.decision_priority_counts)
        .sort((a, b) => Number(b[1] || 0) - Number(a[1] || 0));
      ...
      const priorityList = priorityCounts.map(([label, count]) => (
        `<li>${esc(label)} <span class="muted">(${esc(fmt(count))})</span></li>`
      )).join('');
    ...
    function renderPlatformScope(inventory: PlatformInventory): void {
      if (!platformEl) return;
      const platformCounts = mapEntries(inventory.platform_counts)
        .sort((a, b) => Number(b[1] || 0) - Number(a[1] || 0))
        .slice(0, 6);
      ...
      const platformCards = platformCounts.map(([platform, count]) => {
        ...
        const topAlignment = mapEntries(alignmentCounts)
          .sort((a, b) => Number(b[1] || 0) - Number(a[1] || 0))
          .slice(0, 3)
          .map(([label, value]) => `${label}=${fmt(value)}`)
          .join(', ') || '--';
        const topDecision = mapEntries(decisionCounts)
          .sort((a, b) => Number(b[1] || 0) - Number(a[1] || 0))
          .slice(0, 2)
          .map(([label, value]) => `${label}=${fmt(value)}`)
          .join(', ') || '--';
    ...
    function renderAskWhyBreakdown(inventory: AskWhyInventory): void {
      if (!askWhyEl) return;
      const issueCounts = mapEntries(inventory.issue_kind_counts)
        .sort((a, b) => Number(b[1] || 0) - Number(a[1] || 0))
        .slice(0, 8);
      const platformCounts = mapEntries(inventory.platform_counts)
        .sort((a, b) => Number(b[1] || 0) - Number(a[1] || 0))
      ...
      const issueList = issueCounts.map(([label, count]) => {
        const platformMix = mapEntries(toRecord(issuePlatformCounts[label]))
          .sort((a, b) => Number(b[1] || 0) - Number(a[1] || 0))
          .slice(0, 3)
    ...
    function renderRemediation(summary: RemediationSummary): void {
      if (!remediationEl) return;
      const lanes = asRows<PriorityLane>(summary.priority_lanes);
      ...
      const laneCards = lanes.map((lane) => {
        ...
        const drillParams: Record<string, unknown> = { limit: limitSelect ? limitSelect.value : '100' };
    ...
    function applyUrl(limit: string, alignment: string, platform: string, pattern: string, query: string, pairCatalog: string, pairSignal: string): void {
    ...
    async function load(): Promise<void> {
      ...
      try {
        ...
        const data = toRecord(body.data) as CheckData;
        const meta = toRecord(body.meta) as CheckMeta;
        ...
        if (metaEl) {
          ...
          const pairFocus = meta.pair_catalog || meta.pair_signal
            ? ` | Pair focus: ${String(meta.pair_catalog || '(empty)')} vs ${String(meta.pair_signal || '(empty)')}`
            : '';
        ...
        renderOperationalSplit(toRecord(data.decision_inventory) as DecisionInventory);
        renderPlatformScope(toRecord(data.platform_inventory) as PlatformInventory);
        renderAskWhyBreakdown(toRecord(data.ask_why_inventory) as AskWhyInventory);
        renderSummary(asRows<SummaryRow>(data.summary));
        renderRemediation(toRecord(data.remediation_summary) as RemediationSummary);
      } catch (err) {
        App.renderPageError(errorEl, {
    ...
    [searchInput, alignmentSelect, platformSelect, patternSelect, limitSelect]
      .filter((node): node is HTMLInputElement | HTMLSelectElement => node !== null)
      .forEach((node) => {
        if (node instanceof HTMLSelectElement) {
          node.addEventListener('change', () => { void load(); });
        }
      });
