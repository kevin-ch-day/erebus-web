...
const root = document.getElementById('perm-google-page') as HTMLElement | null;
...
if (root && window.App && window.PermissionIntel) {
  ...
  const searchEl = document.getElementById('perm-google-search') as HTMLInputElement | null;
  const namespaceEl = document.getElementById('perm-google-namespace') as HTMLInputElement | null;
  ...
  if (!endpoint || !bodyEl || !metaEl || !errorEl) {
    ...
    PI.bindEnterReload([searchEl, namespaceEl], () => { void page.resetAndLoad(); });
