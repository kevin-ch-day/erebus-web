...
const root = document.getElementById('perm-oem-registry-page') as HTMLElement | null;
...
if (root && window.App && window.PermissionIntel) {
  ...
  const searchEl = document.getElementById('oem-search') as HTMLInputElement | null;
  const classEl = document.getElementById('oem-class') as HTMLSelectElement | null;
  ...
  if (!endpoint || !bodyEl || !metaEl || !errorEl) {
    ...
    const page = PI.createReadonlyCatalogPage({
      ...
      renderMetaText: (meta: JsonRecord) => `Showing ${fmtCount(meta.total_count)} namespaces. Default review scope: ${String(meta.default_class_recommended || 'oem').toUpperCase()}.`,
      buildParams: () => ({
      ...
      loadLov: (lov: unknown) => {
        ...
        const classes = Array.isArray(body.namespace_classes) ? body.namespace_classes as Array<Record<string, unknown>> : [];
    ...
    classEl?.addEventListener('change', () => { void page.resetAndLoad(); });
