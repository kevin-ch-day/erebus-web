// @ts-nocheck
import '../modules/perm-triage/permission-triage-table-renderer';
import '../modules/perm-triage/permission-triage-session';

// Ported from legacy JS; tighten types incrementally.

import type { AppSurface, JsonRecord, PermissionIntelSurface } from '../types/app-globals';
import type { PermTriageNamespace, TriageFilters, TriagePaging, TriageRow, TriageStatusOption } from '../types/perm-triage-globals';

const root = document.getElementById('perm-triage-page') as HTMLElement | null;
...
if (root && window.App && window.PermissionIntel && window.PermTriage) {
  ...
  const PermTriage = window.PermTriage as PermTriageNamespace;
  ...
  let activeReviewRows: TriageRow[] = [];
  let governedReviewRows: TriageRow[] = [];
  let ledgerDiagnosticRows: TriageRow[] = [];
  let filteredRows: TriageRow[] = [];
  let activePage: TriagePaging = { page: 1, page_size: Number(defaultLimit), total_count: 0, total_pages: 1, has_more: false };
  let governedPage: TriagePaging = { page: 1, page_size: Number(defaultLimit), total_count: 0, total_pages: 1, has_more: false };
  let ledgerPage: TriagePaging = { page: 1, page_size: Number(defaultLimit), total_count: 0, total_pages: 1, has_more: false };
  let pageMeta: TriagePaging = activePage;
  let healthData: JsonRecord = {};
  const triageStatuses: TriageStatusOption[] = [];
  const triageStatusMap = new Map<string, string>();
