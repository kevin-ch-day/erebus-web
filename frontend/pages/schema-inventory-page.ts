...
type SurfaceRow = JsonRecord & {
  name?: unknown;
  catalog?: unknown;
  catalog_role?: unknown;
  expected_object_kind?: unknown;
  actual_table_type?: unknown;
  analysis_role?: unknown;
  available?: unknown;
  present?: unknown;
  consumer_pages?: unknown;
  missing_columns?: unknown;
};

const root = document.getElementById('schema-inventory-page') as HTMLElement | null;
...
if (root && window.App) {
  ...
  const searchEl = document.getElementById('schema-inventory-search') as HTMLInputElement | null;
  const availabilityEl = document.getElementById('schema-inventory-filter-availability') as HTMLSelectElement | null;
  const roleEl = document.getElementById('schema-inventory-filter-role') as HTMLSelectElement | null;
  ...
  let surfaces: SurfaceRow[] = [];

  function badgeForSurface(surface: SurfaceRow): string {
  ...
  function surfaceMatches(surface: SurfaceRow, search: string, availability: string, role: string): boolean {
  ...
  function renderSummary(payload: JsonRecord, meta: JsonRecord): void {
    ...
    if (metaEl) metaEl.textContent = `Loaded: ${formatUtc(new Date().toISOString())}`;
  }
  ...
  async function load(): Promise<void> {
    ...
    try {
      ...
      surfaces = Array.isArray(payload.surfaces) ? payload.surfaces as SurfaceRow[] : [];
