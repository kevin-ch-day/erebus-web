(() => {
  ...
  function familyAlignmentBadge(rawAlignment) {
    ...
    const label = alignment === '' ? '--' : alignment;
    return '<span class="badge ' + cls + '">' + esc(label) + '</span>';
  }

  SamplesPage.applyColumnsView = (tableEl, value) => {
    if (!tableEl) return;
    tableEl.classList.toggle('simple', value === 'simple');
    tableEl.classList.toggle('detailed', value === 'detailed');
  };

  SamplesPage.renderRows = (rows, options) => {
    ...
    rows.forEach((row) => {
      ...
      const link = detailBase + (detailBase.includes('?') ? '&' : '?') + 'sample_id=' + encodeURIComponent(sampleId);
      ...
      tr.innerHTML = `
        ...
        <td>${familyAlignmentBadge(familyAlignment)}<br><span class="muted">${esc(signalFamily)}</span></td>
  ...
  SamplesPage.renderMeta = (payload, options) => {
    ...
    metaEl.textContent = `Total: ${total} | Page size: ${pageSize}${activeText}`;
