(function() {
  ...
  const root = document.getElementById("family-taxonomy-queue-page");
  if (root && window.App) {
    const pageRoot = root;
    ...
    if (endpoint) {
      ...
      `).join("");
        ...
        `;
      }, rowMatchesFilter = function(row, query) {
        const haystack = [
          row.sample_id,
          row.family_label,
        ...
        rowsBodyEl.innerHTML = rows.map((row) => {
          ...
          const rowStateClass = String(row.decision_priority || "").toLowerCase() === "high" ? "queue-row-priority-high" : String(row.decision_priority || "").toLowerCase() === "medium" ? "queue-row-priority-medium" : "queue-row-priority-low";
          const targetFamily = String(row.suggested_target_family || "").trim();
          const targetLine = targetFamily !== "" ? `<div class="queue-row-meta"><span class="queue-row-meta-label">Target</span> ${esc(targetFamily)}</div>` : "";
          ...
          const signalHint = signalLabel !== signalName ? `<div class="queue-row-meta">${esc(compactCopy(signalLabel, "--", 32))}</div>` : "";
          return `
      ...
      }, applyUrl = function(limit, alignment, platform, pattern, query, pairCatalog, pairSignal, fixAction, targetFamily, decisionMode) {
      ...
      const searchInput = document.getElementById("family-taxonomy-queue-search");
      const alignmentSelect = document.getElementById("family-taxonomy-queue-alignment");
      const platformSelect = document.getElementById("family-taxonomy-queue-platform");
      const patternSelect = document.getElementById("family-taxonomy-queue-pattern");
      const limitSelect = document.getElementById("family-taxonomy-queue-limit");
      const rowFilterInput = document.getElementById("family-taxonomy-queue-row-filter");
      const refreshBtn = document.getElementById("family-taxonomy-queue-refresh");
      const exportBtn = document.getElementById("family-taxonomy-queue-export");
      ...
      async function load() {
        ...
        try {
          const buildQueueUrl = (decisionModeOverride) => {
          ...
          let data = toRecord(body.data);
          let meta = toRecord(body.meta);
          let queueRows = asRows(data.rows);
          ...
          if (hasExactPairFocus && decisionMode !== "" && queueRows.length === 0) {
            ...
            if (fallbackRes.ok) {
              ...
              const fallbackData = toRecord(fallbackBody.data);
              const fallbackRows = asRows(fallbackData.rows);
              if (fallbackRows.length > 0) {
                const fallbackMeta = toRecord(fallbackBody.meta);
          ...
          if (metaEl) {
            ...
            const recoveryFocus = meta.requested_decision_mode && meta.recovered_decision_mode && meta.requested_decision_mode !== meta.recovered_decision_mode ? ` | Recovered: ${meta.requested_decision_mode} -> ${meta.recovered_decision_mode}` : "";
          ...
          if (rowsCopyEl) {
            rowsCopyEl.textContent = meta.pair_catalog || meta.pair_signal ? "Rows ordered for this focused conflict. Use the quick filter here before opening grouped planning work." : "Rows ordered by repair priority. Use the quick filter here before opening grouped planning work.";
          }
      ...
      [searchInput, alignmentSelect, platformSelect, patternSelect, limitSelect].filter((el) => el !== null).forEach((el) => {
