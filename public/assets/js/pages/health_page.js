(function() {
  ...
  if (root && window.App) {
    ...
    if (endpoint) {
      let fmt = function(value, fallback = "NULL") {
        ...
        const cards = [
          ...
          },
          {
            ...
            body: pendingLike > 0 ? `Retry wait + processing pressure is visible. Check intake backlog before assuming the pipeline is quiet.` : `No queue-like residue is visible here. Eligible now: ${esc(metricFmt(metrics.eligible_now))}.`,
      ...
      `).join("");
      }, setStoplight = function(isHold, holdUntil, holdReason) {
      ...
      }, renderSchemaGuard = function(guard) {
      ...
      }, renderSchemaInventory = function(inventory) {
        ...
        if (schemaInventorySummaryEl) {
          schemaInventorySummaryEl.textContent = missingSurfaces || missingColumns ? `Web knows ${total} DB surfaces; ${missingSurfaces} missing surfaces and ${missingColumns} missing columns need attention.` : `Web knows ${total} DB surfaces; all expected columns are present.`;
        }
      }, renderVtSurfaceSummary = function(summary) {
        ...
        vtSurfaceSummaryEl.textContent = missing > 0 ? `Canonical vendor, projection, delta, and signal surfaces are incomplete: ${missingNames.slice(0, 4).join(", ")}${missingNames.length > 4 ? " ..." : ""}.` : "Canonical vendor, projection, delta, and signal surfaces are present.";
      }, renderFamilyTaxonomySummary = function(summary) {
      ...
      }, renderWorkflowDebt = function(workflowDebt) {
        ...
        const unexpectedLive = asRows(workflowDebt.unexpected_live_triage_statuses);
        ...
        const samples = asRows(guard.sample);
