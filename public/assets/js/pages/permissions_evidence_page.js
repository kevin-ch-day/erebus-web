(function() {
  ...
  if (root && window.App && window.PermissionIntel) {
    let updateReviewLink = function(permission) {
    ...
    }, updateUrl = function(permission, limit) {
    ...
    async function loadEvidence(permission) {
      ...
      try {
        ...
        const meta = res.body.meta && typeof res.body.meta === "object" ? res.body.meta : {};
        ...
        rows.forEach((row) => {
          ...
          const runStatus = row.stopped_reason ? `Stopped: ${fmt(row.stopped_reason)}` : Number(row.ok_count || 0) > 0 ? "OK" : "--";
          ...
          const observedHtml = observedSecondary ? `<div>${esc(observedPrimary)} (${esc(displayTz)})</div><div class="muted">${esc(observedSecondary)} (${esc(secondaryTz)})</div>` : `<div>${esc(observedPrimary)} (${esc(displayTz)})</div>`;
          const tr = document.createElement("tr");
          tr.innerHTML = `
          <td>${sampleUrl ? `<a class="table-link" href="${esc(sampleUrl)}">${esc(sampleText)}</a>${triageBadge}` : `${esc(sampleText)}${triageBadge}`}</td>
