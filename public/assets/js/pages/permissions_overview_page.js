(() => {
  ...
  function updateNextAction(unknownPct, operatorSummary, session) {
  ...
  function renderHealth(health, meta) {
    ...
    // keep the health card focused on health; final operator guidance is set after the full payload loads
  }

  function renderSignals(health, taxonomy) {
    ...
    if (unknownTrendEl) {
      ...
      if (delta === null || delta === undefined) {
        ...
        const sign = delta > 0 ? '+' : '';
        unknownTrendEl.textContent = `${formatPct(health.unknown_pct_7d)} (${sign}${delta.toFixed(1)}%)`;
      }
    }
  }
  ...
  function renderStatusModel(statusModel) {
    ...
    unexpectedStatuses.forEach((statusKey) => {
      ...
      li.textContent = `Unexpected live triage status: ${statusKey}`;
  ...
  function renderMaintenance(health, maintenance, statusModel) {
    ...
    if (unknownDelta !== null && unknownDelta > 0) {
      triggers.push(`Unknown rate increased by ${unknownDelta.toFixed(1)}% over 7 days.`);
    }
    const unexpectedStatuses = Array.isArray(statusModel && statusModel.unexpected_live_triage_statuses)
      ? statusModel.unexpected_live_triage_statuses
      : [];
  ...
  function renderTopUnknowns(rows) {
  ...
  function renderGovernedUnknowns(rows) {
  ...
  function renderLedgerDiagnostics(rows) {
  ...
  function renderRollupGuard(guard) {
    ...
    if (samples.length > 0 && rollupGuardDetailsEl) {
      ...
      samples.slice(0, 10).forEach((row) => {
        ...
        const lag = row.lag_seconds ? `${Math.round(row.lag_seconds / 86400)}d` : '0d';
  ...
  function renderBacklog(statusCounts, queue, health, metrics, operatorSummary, maintenance) {
    if (!backlogNewEl || !backlogAospEl || !backlogOemEl || !backlogQueuedEl) return;
    const metricData = metrics || {};
    const summary = operatorSummary || {};
    const counts = metricData.triage_status_counts || statusCounts || {};
    ...
    if (backlogResolvedOemEl) {
      backlogResolvedOemEl.textContent = formatCount(counts.resolved_oem || metricData.resolved_oem_count || healthData.resolved_oem_count || 0);
    }
    backlogQueuedEl.textContent = formatCount(
      (metricData.queue_counts && (
        metricData.queue_counts.queued_current_unknown
        ?? metricData.queue_counts.queued
      ))
  ...
  function gapReasonLabel(value) {
    ...
    return labels[String(value || '')] || String(value || 'Review context');
  }
  ...
  function filteredClassificationGapRows() {
    ...
    return classificationGapRows.filter((row) => String(row.workflow_state || '').toLowerCase() === classificationGapFilter);
  }

  function aggregateClassificationGapSamples(rows) {
    ...
    return Array.from(sampleMap.values())
      .map((entry) => {
        entry.attack_rows.sort((a, b) => {
          if (b.max_mapping_strength_rank !== a.max_mapping_strength_rank) {
            return b.max_mapping_strength_rank - a.max_mapping_strength_rank;
          }
  ...
  function renderClassificationGapFilterState(summary) {
  ...
  function priorityBadge(priority) {
    ...
    return '<span class="badge muted">' + esc(priority || 'Low') + '</span>';
  }

  function renderClassificationGapsPayload(body) {
  ...
  async function loadClassificationGaps() {
    ...
    try {
      ...
      renderClassificationGapsPayload(res.body || {});
    } catch (e) {
      if (classificationGapsSummaryEl) {
        classificationGapsSummaryEl.textContent = 'Failed to load classification gaps.';
      }
      classificationGapsBodyEl.innerHTML = '<tr><td colspan="5" class="err">' + esc(e && e.message ? e.message : String(e)) + '</td></tr>';
    }
  }
  ...
  if (classificationGapFiltersEl) {
    classificationGapFiltersEl.querySelectorAll('button[data-gap-filter]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        renderClassificationGapsPayload({ data: { gaps: classificationGapRows, summary: classificationGapSummaryRows }, meta: {} });
      });
    });
  }
  ...
  async function loadOverview() {
    ...
    try {
      ...
      renderHealth(data.health || {}, meta);
      renderSignals(data.health || {}, data.taxonomy || {});
      renderStatusModel(data.status_model || {});
      renderMaintenance(data.health || {}, data.maintenance || {}, data.status_model || {});
      renderTopUnknowns(data.current_evidence_review_rows || []);
      renderGovernedUnknowns(data.governed_current_unknown_rows || []);
      renderLedgerDiagnostics(data.ledger_diagnostic_rows || []);
      renderBacklog(
        data.triage_status_counts || {},
        data.queue || {},
        data.health || {},
        data.metrics || {},
      ...
      updateNextAction(data.health && data.health.unknown_pct, data.operator_summary || {}, data.session || {});
      renderRollupGuard(data.rollup_guard || null);
      loadClassificationGaps();
      setPageReady();
    } catch (e) {
      setPageError('Failed to load permission overview.');
