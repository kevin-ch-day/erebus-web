(function() {
  "use strict";
  const root = document.getElementById("stack-audit-page");
  ...
  if (root && window.App) {
    ...
    if (!endpoint) ;
      ...
      let severityBadge = function(value) {
        const key = String(value || "").toLowerCase();
        if (key === "critical") return "badge err";
        if (key === "warn") return "badge warn";
        if (key === "info") return "badge muted";
        return "badge muted";
      }, renderRuntime = function(runtime, capabilities) {
        if (!runtimeEl) return;
        const frontendDeps = asRows(runtime.frontend_dependencies);
      ...
      `;
      }, renderArchitecture = function(rows) {
      ...
      `).join("");
      }, renderCliEntrypoints = function(rows) {
        if (!cliEl) return;
        if (!rows.length) {
      ...
      async function load() {
        ...
        try {
          ...
          const runtime = toRecord(data.runtime);
          runtime.project_root = data.project_root || "";
          renderRuntime(runtime, toRecord(data.capabilities));
          renderArchitecture(asRows(data.architecture_profile));
          renderGaps(asRows(data.gap_inventory));
          renderTracks(asRows(data.upgrade_tracks));
          renderCliEntrypoints(asRows(data.cli_entrypoints));
          renderAnchors(asRows(data.research_anchors));
          if (metaEl) {
            metaEl.textContent = `Loaded: ${formatUtc(meta.generated_at_utc || (/* @__PURE__ */ new Date()).toISOString())}`;
          }
        } catch (err) {
          App.renderPageError(errorEl, {
